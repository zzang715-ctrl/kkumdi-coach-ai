...
import { LecturePlanDraftView } from "@/features/lecturePlans/LecturePlanDraftView";

type LecturePlanPageProps = {
  params: Promise<{
    projectId: string;
  }>;
};

export default async function LecturePlanPage({ params }: LecturePlanPageProps) {
  ...
  return (
    <main className="min-h-screen bg-[#f7f3ea] text-slate-900">
      <WorkHeader
        label="강의 기획서 AI"
        title="강의 기획서 만들기"
        description="강의 목적, 대상, 시간표, 활동 흐름을 하나의 기획서로 정리합니다."
      />
      <section className="mx-auto w-full max-w-7xl px-5 py-10 sm:px-8 lg:px-10">
        <LecturePlanDraftView projectId={projectId} />
      </section>
    </main>
  );
}
